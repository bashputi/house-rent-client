  ...
  const handleDeleteItem = (id) => {
      ...
      Swal.fire({
          ...
          if (result.isConfirmed) {
              fetch(`http://localhost:5002/userbooks/${id}`,{
                  method: 'DELETE'
              })
              .then(res => res.json())
              .then(data => {
      ...
      return (
        ...
        data-aos-duration="2500">
              {/* row  */}
             {
             allRent.length && allRent.map((item, index) => (
                  <tr key={item._id}>
                  <th>
                   {index + 1}
                  </th>
                  <td>
                    <img className="w-16 h-12 rounded-full" src={item.image} alt="" />
                  </td>
                  <td>
                    {item.name}
